module Vertica
  class Column
    ...
    DATA_TYPE_CONVERSIONS = {
      ...
      10  => [:date,         lambda { |s| Date.new(*s.split("-").map(&:to_i)) }],
      11  => [:time,         nil],
      12  => [:timestamp,    lambda { |s| puts "timestamp: #{s}"; Time.parse(s) }],
      13  => [:timestamp_tz, lambda { |s| puts "timestamp_tz: #{s}"; Time.parse(s) }],
      14  => [:interval,     nil],
      15  => [:time_tz,      nil],
